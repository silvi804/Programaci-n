...
try:
    ...
    alerta = '''\033[31mNo se logró encontar el archivo "color.py" en la ruta por defecto.,
Tenga en cuenta que debe estar en la misma carpeta que este programa (archivo ".py")\033[0m'''
    print(alerta)
    color_is_imported = False
except ImportError:
...
# Identificador de otras excepciones posibles (no contempladas).
    ...
    def siguiente(proceso="continuar"):
        ...
        try:
            ...
            mensaje = f'''Digitó {texto_ctrlc}, el comando para salir del programa,\n -Presione {texto_input} para salir.'''
            print(mensaje)
            try:
                entrada = input(
    ...
    def titulo(texto, diccionario):
        ...
        textito = f"{Color.SUBRAY}{texto}"
        longitud = 5
        for categoria in diccionario:
            longitud += len(categoria) + 5

        titulo_subrayado = f"{textito:^{longitud}}"
...
ruta = r'C:\Unal\Programación_2023_S1\Proyecto\Proy_S1_2023'
programa = False

if color_is_imported:

    try:
        with open(ruta+r'\datos.txt', 'r', encoding='utf8') as datos:
            ventas = save_in_diccionary(datos)
    except FileNotFoundError:  # Al no encontrar el archivo.
        try:
            absFilePath = path.abspath(__file__)
            path, filename = path.split(absFilePath)
...
Tenga en cuenta que debe estar en la misma carpeta que este programa (archivo ".py")'''
            print('\033[31m'+alerta+'\033[0m')
        except Exception as excep:
            exception = 'Ocurrió la siguiente excepción: ' + str(type(excep))
            print('\033[31m'+exception+'\033[0m')
        else:
            programa = True
    except Exception as excep:
        exception = ' Ocurrió la siguiente excepción: ' + str(type(excep))
...
if programa:
    ...
    pretty_tabla(ventas, len(ventas['id_producto']),
                 " Programa de cálculo de ventas")
    siguiente()
    ...
    titulo1 = "Número de ventas"
    dic_1 = diccionario_especifico(sup, "número_de_ventas")
    pretty_tabla(dic_1, len(dic_1["id_producto"]), titulo1)
    siguiente()
    ...
    separadorcito = separador_horizontal(len("valor_total")+4)
    ...
    titulo2 = "Total vendido"
    pretty_tabla(dic_2, len(dic_2["id_producto"]), titulo2)
    ...
    titulo3 = "Tabla de contenidos"
    pretty_tabla(sup, len(sup["cantidad_vendida"]), titulo3)
